    ...
    init: function() {//此方法初始化程序
        ...
        this.socket.on('connect', function() {
			...
			that.socket.on('loginSuccess', function() {
				 document.title = '客服中心 | ' + document.getElementById('nicknameInput').value;
			...
			that.socket.on('system', function(nickName, userCount, type) {
				 ...
				 //指定系统消息显示为红色
				 that._displayNewMsg('系统信息', msg, '#ceccce');
				 //将在线人数显示到页面顶部
				 document.getElementById('status').textContent = userCount + (userCount > 1 ? ' users' : ' 人') + '在线';
			 });			
